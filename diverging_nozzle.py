...
def create_nozzle_figure():
    ...
    if jet_effects == True:
        ...
        # ax.scatter(nozzle_series[:, 0], nozzle_series[:, 1], s = 0.1, color = 'blue')
    ...
    ax.set_ylabel("$y$")
    ax.set_xlabel("$x$")
    ax.set_title(r"Mach Number in Shock-Free Diverging Nozzle, $M_\mathrm{inl} = 1.1$, $M_\mathrm{exi} = 2.5$")
    # ax.set_title(r"Charateristic Mesh in Planar Nozzle, $M_\mathrm{inl} = 1.1$, $M_\mathrm{exi} = 2.5$")
    ax.legend(loc = 'upper left')

    fig.tight_layout()
    plt.savefig("Mach Number in Planar Nozzle.png", dpi = 200)
    # plt.savefig("Charateristic Mesh in Planar Nozzle", dpi = 400)
    plt.show()

if __name__ == '__main__':
    N = 50 # characteristic line count
    F = 150 # fill point count
